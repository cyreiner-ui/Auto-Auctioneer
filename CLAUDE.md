...
Gemini vision has a hard monthly spend guardrail (`GEMINI_MONTHLY_ANALYSIS_LIMIT`, tracked in `finder_vision_usage`) enforced via `VisionBudgetError`/`VisionQuotaError` in `lib/finder-service.ts` — items are deferred an hour rather than the run failing when a rate/spend cap is hit. The cap applies to free-mode and paid-mode analyses together (`reserve_finder_vision_usage`), not just paid mode — free-mode traffic isn't actually cost-free once Google's own free-tier quota runs out, it just isn't tracked as spend by this app's `GEMINI_PAID_MODE` flag.
