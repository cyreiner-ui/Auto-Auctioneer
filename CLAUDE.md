...
3. `calculateDeal` qualifies an item if `(price + shipping) / knifeCount <= maxCostPerKnife` (per-keyword override via `finder_keywords.max_cost_per_knife`, else the app-editable default at `/staff/finder/settings`, stored in `finder_pocket_knife_settings` — not an env var).
