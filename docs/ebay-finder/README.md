...
`GEMINI_BATCH_SIZE` controls how many pending items get a vision analysis per scheduler tick (once a minute), and `FINDER_PROCESS_CONCURRENCY` controls how many of them run in parallel. Google's free-tier limits vary by model and change over time — check the live numbers at https://aistudio.google.com under Rate Limits for the exact model in `GEMINI_MODEL`, rather than trusting a cached number here. As of this writing, `gemini-3.1-flash-lite`'s free tier allows 4,000 requests/minute and 150,000 requests/day (a previous version of this doc assumed a much stricter 20 requests/minute / 500 requests/day, which is no longer accurate). `GEMINI_BATCH_SIZE=100` and `FINDER_PROCESS_CONCURRENCY=20` finish a full batch in roughly 5 waves — well under the `/api/finder/tick` route's 60-second serverless timeout — while using only a small fraction of the real rate limit, leaving headroom to raise them further if the queue still backs up. If you do exceed the daily cap, the app defers remaining items by an hour rather than failing (see `VisionQuotaError` handling in `lib/finder-service.ts`).
