...
export default function Home() {
  ...
  const complete = () => { if (!completion) return; const price = parsePrice((document.getElementById("final-price") as HTMLInputElement)?.value || ""); const buyer = (document.getElementById("buyer") as HTMLInputElement)?.value.trim(); if (price < 0 || !buyer) { setNotice("Informe o valor final e o nome do comprador."); return; } updateListing({ ...completion, status: "completed", finalPrice: price, buyer }); setCompletion(null); setNotice("Leilão concluído."); };
...
function CompletedRow({ listing, language }: { listing: Listing; language: "pt" | "en" }) { const en = language === "en"; return <div className="completed-row"><div className="completed-item"><span className="dot completed" /><strong>{listing.title}</strong></div><div><small>{en ? "Date" : "Data"}</small><span>{listing.completedAt || "—"}</span></div><div><small>{en ? "Final sale" : "Venda final"}</small><span>{money(listing.finalPrice || 0)}</span></div><div><small>{en ? "Buyer" : "Comprador"}</small><span>{listing.buyer || "—"}</span></div></div>; }
