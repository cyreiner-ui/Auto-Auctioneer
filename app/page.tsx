...
export default function Home() {
  ...
  const updateListing = (next: Listing) => { setListings((all) => all.map((l) => l.id === next.id ? next : l)); void fetch(`/api/listings/${encodeURIComponent(next.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(next) }); };
  const importListings = async () => {
    ...
    try {
      ...
      await Promise.all(created.map((listing) => fetch("/api/listings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(listing) }))); setListings((all) => [...created, ...all]); setLinks("");
  ...
  useEffect(() => { if (!unlocked) return; fetch("/api/listings").then((response) => response.ok ? response.json() : []).then((remote: Listing[]) => { if (remote.length) setListings(remote); }).catch(() => undefined); }, [unlocked]);
  ...
  if (selected) return <Editor listing={selected} language={language} onBack={() => setSelected(null)} onSave={(next) => { updateListing(next); setSelected(null); setNotice(en ? "Listing saved." : "Listing salvo."); }} onDelete={() => { setListings((all) => all.filter((l) => l.id !== selected.id)); void fetch(`/api/listings/${encodeURIComponent(selected.id)}`, { method: "DELETE" }); setSelected(null); }} />;
