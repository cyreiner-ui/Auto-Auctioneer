...
function StaffCard({ listing, onClick, language }: { listing: Listing; onClick: () => void; language: "pt" | "en" }) { const en = language === "en"; return <button className="listing-card" onClick={onClick}><img src={listing.images[0]?.src} alt={listing.title} /><div className="card-copy"><div className="status-line"><span className={`dot ${listing.status}`} />{listing.status === "draft" ? (en ? "Draft" : "Rascunho") : listing.status === "approved" ? (en ? "Approved" : "Aprovado") : (en ? "Completed" : "Concluído")}</div><h3>{listing.title}</h3>{listing.status === "completed" ? <p className="result">{money(listing.finalPrice || 0)} · {listing.buyer}</p> : <strong className="price">{money(listing.price)}</strong>}</div><span className="arrow">↗</span></button>; }
