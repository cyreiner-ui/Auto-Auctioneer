...
export default function Home() {
  ...
  const cleanupStorage = async () => { setStorageLoading(true); try { await fetch("/api/storage", { method: "POST" }); await loadStorage(); } finally { setStorageLoading(false); } };
  ...
  if (selected) return <Editor listing={selected} language={language} onBack={() => setSelected(null)} onSave={async (next) => { try { await updateListing(next); setSelected(null); setNotice(en ? "Listing saved." : "Listing salvo."); } catch { setNotice(en ? "Could not save this listing." : "Não foi possível salvar este listing."); } }} onDelete={async () => { const response = await fetch(`/api/listings/${encodeURIComponent(selected.id)}`, { method: "DELETE" }); if (response.ok) { setListings((all) => all.filter((l) => l.id !== selected.id)); setSelected(null); } }} />;

  return <main className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">✦</span><div><strong>Knife Auctions</strong><small>{en ? "Auction preparation" : "Preparação de leilões"}</small></div></div><div className="role-switch"><button className={role === "staff" ? "active" : ""} onClick={() => setRole("staff")}>{en ? "Staff" : "Equipe"}</button><button className={role === "auctioneer" ? "active" : ""} onClick={() => setRole("auctioneer")}>{en ? "Auctioneer" : "Leiloeiro"}</button></div>{role === "staff" && <button className="storage-link" onClick={() => { setStorageOpen(true); void loadStorage(); }}>{en ? "Storage" : "Armazenamento"}</button>}<div className="language-switch" aria-label="Language"><button className={language === "pt" ? "active" : ""} onClick={() => setLanguage("pt")}>PT</button><button className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>EN</button></div><button className="avatar" aria-label={en ? "Log out" : "Sair"}>AM</button></header>
    {role === "staff" ? <>
      ...
      <section className="listing-section"><div className="section-title"><div><p className="eyebrow">{en ? "YOUR ITEMS" : "SEUS ITENS"}</p><h2>Listings</h2></div><div className="tabs">{(["draft", "approved", "completed"] as Status[]).map((s) => <button key={s} className={section === s ? "active" : ""} onClick={() => setSection(s)}>{s === "draft" ? (en ? "Drafts" : "Rascunhos") : s === "approved" ? (en ? "Approved" : "Aprovados") : (en ? "Completed" : "Concluídos")}<b>{listings.filter((l) => l.status === s).length}</b></button>)}</div></div><div className={section === "completed" ? "completed-list" : "listing-grid"}>{visible.map((listing) => section === "completed" ? <CompletedRow key={listing.id} listing={listing} language={language} onEdit={() => setSelected(listing)} onDelete={async () => { if (!window.confirm(en ? "Delete this sold item?" : "Excluir este item vendido?")) return; const response = await fetch(`/api/listings/${encodeURIComponent(listing.id)}`, { method: "DELETE" }); if (response.ok) setListings((all) => all.filter((l) => l.id !== listing.id)); }} /> : <StaffCard key={listing.id} listing={listing} onClick={() => setSelected(listing)} language={language} />)}{visible.length === 0 && <div className="empty">{en ? "No items in this section." : "Nenhum item nesta seção."}</div>}</div></section>
    ...
    {storageOpen && <StoragePanel language={language} storage={storage} loading={storageLoading} onClose={() => setStorageOpen(false)} onCleanup={cleanupStorage} />}
  </main>;
}

function StoragePanel({ language, storage, loading, onClose, onCleanup }: { language: "pt" | "en"; storage: { listings: number; images: number; files: number; bytes: number } | null; loading: boolean; onClose: () => void; onCleanup: () => void }) { const en = language === "en"; const mb = storage ? (storage.bytes / 1024 / 1024).toFixed(2) : "0.00"; return <div className="modal-backdrop"><div className="modal storage-modal"><button className="close" onClick={onClose}>×</button><p className="eyebrow">{en ? "STORAGE" : "ARMAZENAMENTO"}</p><h2>{en ? "Space overview" : "Visão do espaço"}</h2>{storage ? <div className="storage-stats"><div><strong>{mb} MB</strong><small>{en ? "stored" : "armazenados"}</small></div><div><strong>{storage.files}</strong><small>{en ? "stored files" : "arquivos"}</small></div><div><strong>{storage.listings}</strong><small>{en ? "listings" : "listings"}</small></div></div> : <p className="muted">{en ? "Loading storage usage…" : "Carregando uso do armazenamento…"}</p>}<p className="muted">{en ? "Cleanup removes image files no longer linked to a listing." : "A limpeza remove imagens que não estão mais ligadas a um listing."}</p><div className="modal-actions"><button onClick={onClose}>{en ? "Close" : "Fechar"}</button><button className="primary" onClick={onCleanup} disabled={loading}>{loading ? (en ? "Cleaning…" : "Limpando…") : (en ? "Clean unused files" : "Limpar arquivos não usados")}</button></div></div></div>; }
