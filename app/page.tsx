...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">✦</span><div><strong>Knife Auctions</strong><small>{en ? "Auction preparation" : "Preparação de leilões"}</small></div></div><div className="role-switch"><button aria-pressed={role === "staff"} className={role === "staff" ? "active" : ""} onClick={() => setRole("staff")}>{en ? "Staff" : "Equipe"}</button><button aria-pressed={role === "auctioneer"} className={role === "auctioneer" ? "active" : ""} onClick={() => setRole("auctioneer")}>{en ? "Auctioneer" : "Leiloeiro"}</button></div>{role === "staff" && <Link className="storage-link" href="/staff/finder">{en ? "Pocket knife finder" : "Buscar ofertas de canivetes"}</Link>}{role === "staff" && <Link className="storage-link" href="/staff/finder/carving-sets">{en ? "Carving sets" : "Conjuntos de trinchar"}</Link>}{role === "staff" && <button className="storage-link" onClick={() => { setStorageOpen(true); void loadStorage(); }}>{en ? "Storage" : "Armazenamento"}</button>}<div className="language-switch" aria-label="Language"><button aria-pressed={language === "pt"} className={language === "pt" ? "active" : ""} onClick={() => setLanguage("pt")}>PT</button><button aria-pressed={language === "en"} className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>EN</button></div><button className="avatar" aria-label={en ? "Log out" : "Sair"} onClick={() => void logOutStaff()}>AM</button></header>
    {role === "staff" ? <>
