...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">✦</span><div><strong>Knife Auctions</strong><small>{en ? "Auction preparation" : "Preparação de leilões"}</small></div></div><div className="role-switch"><button className={role === "staff" ? "active" : ""} onClick={() => setRole("staff")}>{en ? "Staff" : "Equipe"}</button><button className={role === "auctioneer" ? "active" : ""} onClick={() => setRole("auctioneer")}>{en ? "Auctioneer" : "Leiloeiro"}</button></div>{role === "staff" && <Link className="storage-link" href="/staff/finder">{en ? "Deal finder" : "Buscar ofertas"}</Link>}{role === "staff" && <button className="storage-link" onClick={() => { setStorageOpen(true); void loadStorage(); }}>{en ? "Storage" : "Armazenamento"}</button>}<div className="language-switch" aria-label="Language"><button className={language === "pt" ? "active" : ""} onClick={() => setLanguage("pt")}>PT</button><button className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>EN</button></div><button className="avatar" aria-label={en ? "Log out" : "Sair"}>AM</button></header>
    {role === "staff" ? <>
