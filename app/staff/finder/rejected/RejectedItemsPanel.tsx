...
export default function RejectedItemsPanel() {
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/finder/rejected?category=pocket_knife", { cache: "no-store" });
  ...
  return <main className="finder-page">
    <header className="finder-header"><div><Link className="back" href="/staff/finder">← Back to finder</Link><p className="eyebrow">EBAY DISCOVERY</p><h1>Rejected items</h1><p className="muted">Listings the finder looked at but didn&rsquo;t qualify, with why. Useful for checking a deal you found manually against what the finder decided.</p></div></header>
