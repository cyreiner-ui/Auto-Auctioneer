...
export default function GauchoKnifeArchivedPanel() {
  const [data, setData] = useState<Overview | null>(null);
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);

  const load = useCallback(async () => {
    const response = await fetch("/api/finder/archived?category=gaucho_knife", { cache: "no-store" });
  ...
  return <main className="finder-page">
    <header className="finder-header"><div><Link className="back" href="/staff/finder">← Back to finder</Link><p className="eyebrow">EBAY DISCOVERY</p><h1>Archived gaucho knives</h1><p className="muted">Deals you&rsquo;ve already handled. Restore one to see it again, or delete it for good.</p></div></header>
