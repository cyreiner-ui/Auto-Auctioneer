...
type Overview = {
  ...
  settings: { mateGourdKeywordSearchEnabled: boolean };
};
...
export default function MateGourdSettingsPanel() {
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/finder?category=mate_gourd", { cache: "no-store" });
  ...
  const setNegativeEnabled = (keyword: Keyword, enabled: boolean) => request("/api/finder/mate-gourd-negative-keywords", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: keyword.id, enabled }) });
  ...
  const saveSchedule = (patch: Partial<Schedule>) => request("/api/finder/schedule", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category: "mate_gourd", ...patch }) });
  const setKeywordSearchEnabled = (enabled: boolean) => request("/api/finder/mate-gourd-settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ keyword_search_enabled: enabled }) });
  const setProcessingPaused = (paused: boolean) => request("/api/finder/processing-paused", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category: "mate_gourd", paused }) });
  ...
  const uploadReferenceImage = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch("/api/finder/mate-gourd-reference-images", { method: "POST", body });
      const payload = await response.json();
  ...
  const removeReferenceImage = (id: string) => { if (window.confirm("Remove this reference photo?")) void request(`/api/finder/mate-gourd-reference-images?id=${encodeURIComponent(id)}`, { method: "DELETE" }); };
  ...
  return <main className="finder-page">
    <header className="finder-header"><div><Link className="back" href="/staff/finder">← Back to deal finder</Link><p className="eyebrow">EBAY DISCOVERY</p><h1>Maté gourd search settings</h1><p className="muted">Discovery leads with a live eBay image search against your reference photos below — no positive keyword is required to reach a candidate, since sellers usually list these with generic wording like &ldquo;mate cup&rdquo; or &ldquo;gourd cup.&rdquo; Every candidate is judged against the specific gourd shown in your reference photos, not just &ldquo;is this a mate gourd.&rdquo; The negative-keyword list filters out obvious lookalikes before anything reaches Gemini; the search-term list below is only a secondary net.</p></div></header>
    ...
    {data && <section className="finder-layout">
      <div className="panel finder-keywords">
        ...
        <input aria-label="Label for the next uploaded reference photo" placeholder="Optional label for the next upload (e.g. which angle/detail)" value={newLabel} onChange={(event) => setNewLabel(event.target.value)} style={{ marginTop: 12 }} />
        <div className="photo-grid" style={{ marginTop: 16 }}>
          ...
          {data.referenceImages.length === 0 && <p className="muted">Upload at least one reference photo of the specific gourd you&rsquo;re after before running this finder — Gemini has nothing to compare candidates against otherwise.</p>}
        </div>
      </div>
      <div className="panel finder-keywords"><div className="panel-heading"><div><p className="eyebrow">SEARCH TERMS</p><h2>Keyword supplement</h2></div></div>
        <p className="muted">Secondary discovery only — the image search above is primary.</p>
        <label className="keyword-toggle"><input type="checkbox" checked={data.settings.mateGourdKeywordSearchEnabled} disabled={busy} onChange={(event) => void setKeywordSearchEnabled(event.target.checked)} /> Run keyword search</label>
        {!data.settings.mateGourdKeywordSearchEnabled && <p className="muted">Keyword search is off — this run only discovers candidates via the image search above. The phrases below are kept, but none of them are searched until this is turned back on.</p>}
        <div className="keyword-list" style={data.settings.mateGourdKeywordSearchEnabled ? undefined : { opacity: 0.5 }}>{data.keywords.map((keyword) => <div className="keyword-row" key={keyword.id}><span>{keyword.phrase}</span><label className="keyword-toggle"><input type="checkbox" checked={keyword.enabled} disabled={busy || !data.settings.mateGourdKeywordSearchEnabled} onChange={(event) => void setKeywordEnabled(keyword, event.target.checked)} /> Active</label><button className="danger" aria-label={`Delete "${keyword.phrase}"`} disabled={busy} onClick={() => { if (window.confirm(`Delete "${keyword.phrase}"?`)) void request(`/api/finder/keywords?id=${encodeURIComponent(keyword.id)}`, { method: "DELETE" }); }}>Delete</button></div>)}</div>
      ...
      <div className="panel finder-schedule">
        ...
        <p className="muted">Runs in America/New_York time. Scheduled independently from the other finders.</p>
      </div>
      <div className="panel finder-notify">
        ...
        <p className="muted">Shared with the other finders — one recipient list and mode for all of them, but maté-gourd deals always go out in their own separate email.</p>
