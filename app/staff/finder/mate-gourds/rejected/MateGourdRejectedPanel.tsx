...
export default function MateGourdRejectedPanel() {
  const [data, setData] = useState<Overview | null>(null);
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);

  const load = useCallback(async () => {
    const response = await fetch("/api/finder/rejected?category=mate_gourd", { cache: "no-store" });
  ...
  return <main className="finder-page">
    <header className="finder-header"><div><Link className="back" href="/staff/finder">← Back to finder</Link><p className="eyebrow">EBAY DISCOVERY</p><h1>Rejected maté gourds</h1><p className="muted">Listings the finder looked at but didn&rsquo;t match, with why. Useful for checking a listing you found manually against what the finder decided.</p></div></header>
