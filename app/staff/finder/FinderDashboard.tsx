...
export default function FinderDashboard() {
  ...
  return <main className="finder-page">
    ...
    {error && <div className="notice finder-error" role="status" aria-live="polite" aria-atomic="true">{error.includes("finder_") || error.includes("relation") ? "Apply the finder database migration, then reload this page." : error}</div>}
    ...
    {data && <>
      ...
      <section className="finder-layout">
        ...
        <aside className="panel finder-budget"><p className="eyebrow">MONTHLY GUARDRAIL</p><h2>{data.budget.mode === "paid" ? `${data.budget.paidAnalyses.toLocaleString()} analyses` : "Free tier"}</h2><p className="muted">{data.budget.mode === "paid" ? `${data.budget.remaining.toLocaleString()} remain before the 50,000-analysis stop.` : "If Gemini reaches the free limit, new items stay queued for a later run."}</p><div className="budget-meter" role="progressbar" aria-valuenow={budgetPercent} aria-valuemin={0} aria-valuemax={100} aria-label="Monthly Gemini analysis budget used"><span style={{ width: `${budgetPercent}%` }} /></div><small>Conservative maximum used: {usd(data.budget.projectedMaximum)} / $50.00</small></aside>
      </section>
