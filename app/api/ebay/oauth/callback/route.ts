...
export async function GET(request: Request) {
  ...
  const clientId = process.env.EBAY_CLIENT_ID; const clientSecret = process.env.EBAY_CLIENT_SECRET; const redirectUri = process.env.EBAY_OAUTH_REDIRECT_URI;
  ...
  const { error } = await supabaseAdmin.from("ebay_accounts").insert({ label: "Connected eBay account", marketplace: process.env.EBAY_MARKETPLACE_ID || "EBAY_US", status: "connected", refresh_token_ciphertext: encryptedToken, token_expires_at: token.refresh_token_expires_in ? new Date(Date.now() + token.refresh_token_expires_in * 1000).toISOString() : null });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  const response = NextResponse.redirect(new URL("/", request.url)); response.cookies.delete("ebay_oauth_state"); return response;
}
