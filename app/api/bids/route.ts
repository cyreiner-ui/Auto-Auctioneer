...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("bid_lots").insert({ ebay_item_id: bid.ebayItemId, ebay_url: bid.ebayUrl, title: bid.title, account_id: bid.accountId, execution_mode: bid.executionMode, max_bid: bid.maxBid, currency: bid.currency, all_in_budget: bid.allInBudget, auction_end_at: bid.auctionEndAt, bid_window_start: bid.bidWindowStart, bid_window_end: bid.bidWindowEnd, timezone: "America/New_York", status: "draft" }).select("*, ebay_accounts(id, label, marketplace, status)").single();
