...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (action !== undefined && action !== "arm" && action !== "disarm") return NextResponse.json({ error: "Action must be arm or disarm." }, { status: 400 });
  const { data: current, error: currentError } = await supabaseAdmin.from("bid_lots").select("id, status, account_id, ebay_item_id, ebay_url, title, max_bid, currency, all_in_budget, auction_end_at, bid_window_start, bid_window_end").eq("id", id).single();
  ...
  const fieldMap: Record<string, [string, unknown]> = {
    accountId: ["account_id", validation.value.accountId], maxBid: ["max_bid", validation.value.maxBid], currency: ["currency", validation.value.currency], allInBudget: ["all_in_budget", validation.value.allInBudget], auctionEndAt: ["auction_end_at", validation.value.auctionEndAt], bidWindowStart: ["bid_window_start", validation.value.bidWindowStart], bidWindowEnd: ["bid_window_end", validation.value.bidWindowEnd], title: ["title", validation.value.title], ebayUrl: ["ebay_url", validation.value.ebayUrl]
  };
