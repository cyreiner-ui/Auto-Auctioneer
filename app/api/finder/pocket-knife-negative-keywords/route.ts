...
// A staff-editable, post-fetch text filter applied to every pocket-knife candidate before
// analyzeListingText runs (see lib/finder-service.ts's initialRow) — mirrors
// app/api/finder/gaucho-negative-keywords/route.ts, but against finder_pocket_knife_negative_keywords.
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("finder_pocket_knife_negative_keywords").insert({ phrase }).select("*").single();
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("finder_pocket_knife_negative_keywords").update(changes).eq("id", body.id).select("*").single();
  if (error) return NextResponse.json({ error: error.code === "23505" ? "That negative keyword already exists." : error.message }, { status: 400 });
  return NextResponse.json(data);
}
...
export async function DELETE(request: Request) {
  ...
  const { error } = await supabaseAdmin.from("finder_pocket_knife_negative_keywords").delete().eq("id", id);
