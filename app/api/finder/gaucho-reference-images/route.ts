...
// discovery calls and the Gemini vision comparison prompt (see lib/gaucho-knife-finder.ts). Mirrors
// app/api/storage/upload/route.ts's upload/signed-URL pattern, but against a dedicated private
// bucket rather than the unrelated listing-images bucket that backs the consignment-listing feature.
export async function POST(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("finder_reference_images").insert({ category: "gaucho_knife", storage_path: path, label }).select("*").single();
